package com.sawcao.beanoperation;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.sawcao.beanoperation.copier.*;
import com.sawcao.beanoperation.map.CachedBeanMap;
...
public class BeanHelper {
	...
	public BeanHelper(final IInstantiationStrategy instantiationStrategy) {
	...
	/**
	 * this makes the cloner to set a transient field to null upon copier.
	 *
	...
	/**
	 * instead of copier these classes will set the field to null
	 *
	 * @param c the classes to nullify during copier
	 */
	...
	public <T> T deepClone(final T o) {
		...
		try {
			...
			throw new CloningException("error during copier of " + o, e);
		}
	}
	...
	public void deepCloneDiffClass(Object src, Object dest){
		if (src != null)
			if (cloningEnabled)
				if (dumpCloned != null) {
					dumpCloned.startCloning(src.getClass());
				}
				final Map<Object, Object> clones = new IdentityHashMap<Object, Object>(16);
				try {
					CachedBeanCopier.copy(cloneInternal(src, clones), dest);
				} catch (final IllegalAccessException e) {
					throw new CloningException("error during copier of " + src, e);
				}
	}
	...
	public Map beanToMap(Object bean){
		...
		return CachedBeanMap.toMap(bean);
	}

	public <T> T mapToBean(Map map,T bean){
		if (map == null) return null;
		return CachedBeanMap.toBean(map,bean);
	}
    ...
    public Object jsonToBean(String jsonStr,Class beanClass){
        if(objectMapper == null){objectMapper = new ObjectMapper();}
        try {
	 ...
	 * @return true to mark clz as immutable and skip copier it
	 */
		...
		for (final Field field : fields) {
			...
			if (!Modifier.isStatic(modifiers)) {
				if (!(nullTransient && Modifier.isTransient(modifiers))) {
					...
					//applyCloningStrategy() provide deepClone
	 ...
	 */
	public static BeanHelper standard() {
		return new BeanHelper();
	}

	/**
	 * @return if BeanHelper lib is in a shared jar folder for a container (i.e. tomcat/shared), then
	 * 		this method is preferable in order to instantiate cloner. Please
	 * 		see https://code.google.com/p/cloning/issues/detail?id=23
	 */
	public static BeanHelper shared() {
		return new BeanHelper(new ObjenesisInstantiationStrategy());
	}
